...
int main(int argc, char **argv) {
  ...
  for(kernel=0;kernel<NUM_KERNELS;kernel++) {
     ...
     for(events=0;events<NUM_EVENTS;events++) {
        ...
        for(run=0;run<NUM_RUNS;run++) {
           sprintf(filename,"results/read_raw/%s/%s/%d/null_test.%d",
  ...
  for(kernel=0;kernel<NUM_KERNELS;kernel++) {
	 ...
	 maxy=average[kernel][events]+stdev[kernel][events];
       ...
       if ( (((average[kernel][events]+stdev[kernel][events])) < miny) &&
	    (((average[kernel][events]+stdev[kernel][events])) > 1 ) ) {
	   miny=average[kernel][events]+stdev[kernel][events];
       }
     }
  }
  maxy=( ((maxy/1000)+1)*1000)/1000000;
  miny=( ((miny/1000)+1)*1000)/1000000;
  ...
  printf("\n");
  printf("(* Y-Axis *)\n");
  printf("yaxis size 2 min %lld max %lld\n",miny,maxy);
  printf("grid_gray 0.9 grid_lines\n");
  //printf("mgrid_gray 0.9 mgrid_lines\n");
  printf("label font Helvetica fontsize %d  : Time (ms)\n",FONTSIZE);
  ...
  printf("title font Helvetica fontsize %d y %lf : "
	 "%s - 128 Reads in MMM\n",
	 FONTSIZE+2,
	 (double)maxy+((double)maxy/64.0),machine);
  printf("\n");
  ...
  for(kernel=0;kernel<NUM_KERNELS;kernel++) {
    ...
    //if (times[kernel][0][0]==0) continue;
     ...
     for(events=0;events<NUM_EVENTS;events++) {
	   ...
	   (double)average[kernel][events]/1000000.0,
	   (double)(average[kernel][events]-stdev[kernel][events])/1000000.0,
	   (double)(average[kernel][events]+stdev[kernel][events])/1000000.0,
	   (double)stdev[kernel][events]/1000000.0,
	   (double)min[kernel][events]/1000000.0,
	   (double)max[kernel][events]/1000000.0);
