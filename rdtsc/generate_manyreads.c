...
static int generate_results(char *directory, int type, int num) {
	...
	if (type==KERNEL_PERF_EVENT_RDPMC) {
		sprintf(dirname,"%s/%s-rdpmc-read_many",directory,uname_buf.release);
	} else if (type==KERNEL_PERF_EVENT_RDPMC_MAP_POPULATE) {
		sprintf(dirname,"%s/%s-rdpmc-map_populate-read_many",directory,uname_buf.release);
	} else if (type==KERNEL_PERF_EVENT_RDPMC_TOUCH) {
		sprintf(dirname,"%s/%s-rdpmc-touch-read_many",directory,uname_buf.release);
	} else if (type==KERNEL_PERFCTR) {
		sprintf(dirname,"%s/%s-perfctr-read_many",directory,uname_buf.release);
	} else if (type==KERNEL_PERFMON2) {
		sprintf(dirname,"%s/%s-perfmon2-read_many",directory,uname_buf.release);
	} else if (type==KERNEL_PERF_EVENT_STATIC) {
		sprintf(dirname,"%s/%s-static-read_many",directory,uname_buf.release);
	} else if (type==KERNEL_PERF_EVENT_SYSCALL) {
		sprintf(dirname,"%s/%s-syscall-read_many",directory,uname_buf.release);
	} else if (type==KERNEL_PERF_EVENT_SYSCALL_STATIC) {
		sprintf(dirname,"%s/%s-syscall_static-read_many",directory,uname_buf.release);
	} else if (type==KERNEL_PERF_EVENT_RDPMC_STATIC) {
		sprintf(dirname,"%s/%s-rdpmc_static-read_many",directory,uname_buf.release);
	} else if (type==KERNEL_PERF_EVENT_RDPMC_TOUCH_STATIC) {
		sprintf(dirname,"%s/%s-rdpmc-touch_static-read_many",directory,uname_buf.release);
	} else if (type==KERNEL_PERF_EVENT_RDPMC_MAP_POPULATE_STATIC) {
		sprintf(dirname,"%s/%s-rdpmc-map_populate_static-read_many",directory,uname_buf.release);
	} else {
		sprintf(dirname,"%s/%s-read_many",directory,uname_buf.release);
	}
   ...
   if (type==KERNEL_PERF_EVENT) {
      sprintf(temp_string2,"taskset 0x1 ./rdtsc_null_pe.10 %d ",num);
      for(i=0;i<num;i++) {
      ...
      sprintf(temp_string2,"taskset 0x1 ./rdtsc_null_pe_static.10 %d ",num);
      for(i=0;i<num;i++) {
         sprintf(temp_string,"0x%x ",event_table->event[i].event);
         strcat(temp_string2,temp_string);
      }
      ...
      sprintf(temp_string2,"taskset 0x1 ./rdtsc_null_pe_syscall.10 %d ",num);
      for(i=0;i<num;i++) {
         sprintf(temp_string,"0x%x ",event_table->event[i].event);
         strcat(temp_string2,temp_string);
      }
      ...
      sprintf(temp_string2,"taskset 0x1 ./rdtsc_null_pe_syscall_static.10 %d ",num);
      for(i=0;i<num;i++) {
         sprintf(temp_string,"0x%x ",event_table->event[i].event);
         strcat(temp_string2,temp_string);
      }
      ...
      sprintf(temp_string2,"taskset 0x1 ./rdtsc_null_pe_rdpmc.10 %d ",num);
      for(i=0;i<num;i++) {
         sprintf(temp_string,"0x%x ",event_table->event[i].event);
         strcat(temp_string2,temp_string);
      }
      ...
      sprintf(temp_string2,"taskset 0x1 ./rdtsc_null_pe_rdpmc_static.10 %d ",num);
      for(i=0;i<num;i++) {
         sprintf(temp_string,"0x%x ",event_table->event[i].event);
         strcat(temp_string2,temp_string);
      }
      ...
      sprintf(temp_string2,"taskset 0x1 ./rdtsc_null_pe_rdpmc-map_populate.10 %d ",num);
      for(i=0;i<num;i++) {
         sprintf(temp_string,"0x%x ",event_table->event[i].event);
         strcat(temp_string2,temp_string);
      }
      ...
      sprintf(temp_string2,"taskset 0x1 ./rdtsc_null_pe_rdpmc-touch.10 %d ",num);
      for(i=0;i<num;i++) {
         sprintf(temp_string,"0x%x ",event_table->event[i].event);
         strcat(temp_string2,temp_string);
      }
      ...
      sprintf(temp_string2,"taskset 0x1 ./rdtsc_null_pe_rdpmc-map_populate_static.10 %d ",num);
      for(i=0;i<num;i++) {
         sprintf(temp_string,"0x%x ",event_table->event[i].event);
         strcat(temp_string2,temp_string);
      }
      ...
      sprintf(temp_string2,"taskset 0x1 ./rdtsc_null_pe_rdpmc-touch_static.10 %d ",num);
      for(i=0;i<num;i++) {
         sprintf(temp_string,"0x%x ",event_table->event[i].event);
         strcat(temp_string2,temp_string);
      }
      ...
      sprintf(temp_string2,"taskset 0x1 ./rdtsc_null_perfctr.10 %d ",num);
      for(i=0;i<num;i++) {
         sprintf(temp_string,"0x%x ",event_table->event[i].event);
         strcat(temp_string2,temp_string);
      }
      ...
      sprintf(temp_string2,"taskset 0x1 ./rdtsc_null_perfmon2.10 %d ",num);
      for(i=0;i<num;i++) {
