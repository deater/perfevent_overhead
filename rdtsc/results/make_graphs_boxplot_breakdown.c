...
int main(int argc, char **argv) {
	...
	int machine_num=0;
	double maxy;
	double average[NUM_BREAKDOWN_KERNELS],deviation[NUM_BREAKDOWN_KERNELS];
	double median[NUM_BREAKDOWN_KERNELS],twentyfive[NUM_BREAKDOWN_KERNELS],
		seventyfive[NUM_BREAKDOWN_KERNELS];
	...
	/* allocate memory */
	times=calloc(NUM_BREAKDOWN_KERNELS*NUM_EVENTS*NUM_RUNS,sizeof(long long));
	if (times==NULL) {
	...
	/* read in data */
	plot_type=read_data(argv[1],machine_num,argv[3],BREAKDOWN_KERNELS,
			times);
	if (plot_type<0) {
	...
	events=EVENT_TO_PLOT;
  	...
  	/* sort data */
	sort_data(times,events,BREAKDOWN_KERNELS);
	...
	calculate_deviation(times,events,average,deviation,BREAKDOWN_KERNELS);


	/* calculate mins and maxes */
	maxy=calculate_maxy(average,deviation,BREAKDOWN_KERNELS);
	...
	printf("\n");
	printf("(* X-Axis *)\n");
	printf("xaxis size 6.5 min %d max %d\n",-1,NUM_BREAKDOWN_KERNELS); //minx,maxx);
	printf("grid_gray 0.9 grid_lines\n");
	...
	for(i=0;i<NUM_BREAKDOWN_KERNELS;i++) {
		printf("hash_at %d\n",i);
	}
	printf("no_auto_hash_labels\n");
	for(i=0;i<NUM_BREAKDOWN_KERNELS;i++) {
		printf("hash_label at %d : %s\n",i,breakdown_kernels[i].name);
	}
	...
	for(kernel=0;kernel<NUM_BREAKDOWN_KERNELS;kernel++) {

		/* plot standard deviation */
		printf("newcurve ");
		if (breakdown_kernels[kernel].type==INTERFACE_PERFCTR) {
			printf("marktype x linetype none color 1.0 0.0 0.0\n");
		}
		else if (breakdown_kernels[kernel].type==INTERFACE_PERFMON2) {
			printf("marktype x linetype none color 0.0 0.0 1.0\n");
		}
		else if (breakdown_kernels[kernel].type==INTERFACE_PERF_EVENT) {
			printf("marktype x linetype none color 0.0 0.0 0.0\n");
     		}
		else if (breakdown_kernels[kernel].type==INTERFACE_PERF_EVENT_RDPMC) {
