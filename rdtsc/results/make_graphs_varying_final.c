...
int main(int argc, char **argv) {
	...
	int type;
        ...
        double maxy,maxys[3][PLOT_EVENTS];
        double average[3][PLOT_EVENTS][NUM_KERNELS],
		deviation[3][PLOT_EVENTS][NUM_KERNELS];
//        double median[3][PLOT_EVENTS][NUM_KERNELS],
//		twentyfive[3][PLOT_EVENTS][NUM_KERNELS],
  //              seventyfive[3][PLOT_EVENTS][NUM_KERNELS];
	...
	for(type=0;type<3;type++) {
		...
		char type_name[BUFSIZ];

		if (type==0) strcpy(type_name,"read");
		if (type==1) strcpy(type_name,"stop");
		if (type==2) strcpy(type_name,"start");
		...
		/* read data */
		plot_type=read_data(argv[1],machine_num,type_name,VARYING_KERNELS,times);
		if (plot_type<0) {
...
//			VARYING_KERNELS);
			...
			calculate_deviation(times,i,
					average[type][i],deviation[type][i],
					VARYING_KERNELS);

			/* calculate mins and maxes */
			maxys[type][i]=calculate_maxy(average[type][i],
				deviation[type][i],VARYING_KERNELS);
	...
	for(events=0;events<PLOT_EVENTS;events++) {
		if (maxys[0][events]>maxy) maxy=maxys[0][events];
	}
	...
	for(kernel=0;kernel<NUM_VARYING_KERNELS;kernel++) {
		...
		switch(type) {
		case 0:
			printf("marktype xbar linetype solid color 1.0 0.0 0.0\n");
			break;
		case 1:
			printf("marktype xbar linetype solid color 0.0 1.0 0.0\n");
			break;
		case 2:
			printf("marktype xbar linetype solid color 0.0 0.0 1.0\n");
			break;
		}
		...
		for(events=1;events<=4;events++) {
			if (average[events][kernel]!=0) {
				if (type==0) {
				printf("\t%d %.2f (* %s *)\n",
					(kernel*4)+events,
					average[0][events][kernel]+
					average[1][events][kernel]+
					average[2][events][kernel],
					varying_kernels[kernel].name);
				}
				if (type==1) {
				printf("\t%d %.2f (* %s *)\n",
