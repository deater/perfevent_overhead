...
int main(int argc, char **argv) {
	...
	/* allocate memory */
	times=calloc(NUM_GCC_KERNELS*NUM_EVENTS*NUM_RUNS,sizeof(long long));
	if (times==NULL) {
	...
	/* read in data */
	plot_type=read_data(argv[1],machine_num,argv[3],GCC_KERNELS,
			times);
	if (plot_type<0) {
	...
	events=EVENT_TO_PLOT;
  	...
  	/* sort data */
	sort_data(times,events,GCC_KERNELS);
	...
	calculate_deviation(times,events,average,deviation,GCC_KERNELS);


	/* calculate mins and maxes */
	maxy=calculate_maxy(average,deviation,GCC_KERNELS);
	...
	for(kernel=0;kernel<NUM_GCC_KERNELS;kernel++) {

		/* plot standard deviation */
		printf("newcurve ");
		if (gcc_kernels[kernel].type==INTERFACE_PERFCTR) {
			printf("marktype x linetype none color 1.0 0.0 0.0\n");
		}
		else if (gcc_kernels[kernel].type==INTERFACE_PERFMON2) {
			printf("marktype x linetype none color 0.0 0.0 1.0\n");
		}
		else if (gcc_kernels[kernel].type==INTERFACE_PERF_EVENT) {
			printf("marktype x linetype none color 0.0 0.0 0.0\n");
     		}
		else if (gcc_kernels[kernel].type==INTERFACE_PERF_EVENT_RDPMC) {
