...
int main(int argc, char **argv) {
  ...
  for(kernel=0;kernel<NUM_KERNELS;kernel++) {
     qsort(times[kernel][events],NUM_RUNS,sizeof(long long),comp);
  }

  double median[NUM_KERNELS],twentyfive[NUM_KERNELS],seventyfive[NUM_KERNELS];

  /* calculate median, twentyfive, seventyfive */
  for(kernel=0;kernel<NUM_KERNELS;kernel++) {
     median[kernel]=times[kernel][events][NUM_RUNS/2];
     twentyfive[kernel]=times[kernel][events][NUM_RUNS/4];
     seventyfive[kernel]=times[kernel][events][(NUM_RUNS*3)/4];
  }
  ...
  for(kernel=0;kernel<NUM_KERNELS;kernel++) {
    ...
    /* plot standard deviation */
     ...
     for(i=0;i<NUM_RUNS;i++) {
       if (times[kernel][events][i]>average[kernel]+deviation[kernel]) {
	  printf("\t%d %lld\n",kernel,times[kernel][events][i]);
       }
       if (times[kernel][events][i]<average[kernel]-deviation[kernel]) {
	  printf("\t%d %lld\n",kernel,times[kernel][events][i]);
       }
