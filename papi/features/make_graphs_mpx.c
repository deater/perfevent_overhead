...
int main(int argc, char **argv) {
  ...
  for(kernel=0;kernel<NUM_KERNELS;kernel++) {
     ...
     for(events=0;events<NUM_EVENTS;events++) {
        ...
        for(run=0;run<NUM_RUNS;run++) {
           sprintf(filename,"results/mpx_papi/%s/%s/%d/mpx_papi.%d",
  ...
  printf("\n");
  printf("(* Y-Axis *)\n");
  printf("yaxis size 0.75 min 0 max %lld\n",maxy);
  ...
  printf("\n");
  printf("(* X-Axis *)\n");
  printf("xaxis size 3 min 0 max 16\n");
  printf("hash_labels font Helvetica fontsize %d\n",FONTSIZE);
  printf("label font Helvetica fontsize %d  : Events being Multiplexed\n",
	 FONTSIZE);
  printf("\n");
  printf("(* Title *)\n");
  printf("title font Helvetica fontsize %d y %lf : %s\n",
	 FONTSIZE+2,(double)maxy+((double)maxy/8.0),machine);
  printf("\n");
   
  for(kernel=0;kernel<NUM_KERNELS;kernel++) {

    if (times[kernel][0][0]==-1) {
     ...
     printf("newcurve marktype %s linetype %s color %s\n",
	   ...
	   linetype[kernel],
	   colors[kernel]);
//     printf("label hjl vjc fontsize 16 font Helvetica x 15.2 y %lf : %s\n",
